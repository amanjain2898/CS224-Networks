...
int main(){
	...
	for(int p = 1; p < n+1; p++){
		...
		for(int k=0;k<26;k++){
			...
			if(position2 != b[p].n_ports.end()){
				char c = k+65;
				cout << c << "-NP ";
				vector<pair<char,int> >::iterator v1 = find(ln[k].np.begin(),ln[k].np.end(),make_pair(c,p));
				if(v1 == ln[k].np.end()){
					ln[k].np.push_back(make_pair(k,p));
					vector<pair<char,int> >::iterator v2 = find(ln[k].dp.begin(),ln[k].dp.end(),make_pair(c,p));
					ln[k].dp.erase(v2);
				}
			}
	...
	for(int i=0;i<26;i++){
		for(int j=0;j<ln[i].np.size();j++){
			int n1 = ln[i].np[j].second;
			vector<int>::iterator position = find(ln[i].adj_bridges.begin(),ln[i].adj_bridges.end(),n1);
			if(position != ln[i].adj_bridges.end()){
				ln[i].adj_bridges.erase(position);
			}
		}
	}

	for(int i=0;i<10;i++){
		for(int j=0;j<b[i].n_ports.size();j++){
			char p = b[i].n_ports[j].first;
			for(int k=0;k<b[i].adj_bridges.size();k++){
				vector<pair<char,int> >::iterator position1 = find(b[i].adj_bridges.begin(),b[i].adj_bridges.end(),make_pair(p,b[i].adj_bridges[k].second));
				if(position1 != b[i].adj_bridges.end()){
					b[i].adj_bridges.erase(position1);
				}
			}
		}
	}
				...
				// if(position == ln[l.id - 65].np.end()){
					if(visited[l.adj_bridges[i]] == false){
						if(fwd[l.adj_bridges[i]].find(host2) == fwd[l.adj_bridges[i]].end()){
							...
							fwd[l.adj_bridges[i]].insert(pair<string,pair<char,int> >(host1,make_pair(l.id,l.adj_bridges[i])));
							// vector<pair<char,int> >::iterator position1 = find(b[l.adj_bridges[i]].n_ports.begin(),b[l.adj_bridges[i]].n_ports.end(),port);
							// if(position1 == b[l.adj_bridges[i]].n_ports.end()){
								if(port.first != l.id && visited[port.second] == false){
									q.push(ln[port.first - 65]);
								}
							// }
						}
					}
	...
	cout << ln[7].adj_bridges.size() <<" " << ln[7].dp.size() << " " << b[7].n_ports.size() << endl;

	// for(int i=0;i<ln[7].adj_bridges.size();i++){
	// 	for(int j=0;j<b[ln[7].adj_bridges[i]].adj_bridges.size();j++){
	// 		cout << b[ln[7].adj_bridges[i]].adj_bridges[j].first << " " << b[ln[7].adj_bridges[i]].adj_bridges[j].second << endl;
	// 	}
	// }
